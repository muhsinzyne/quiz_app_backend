...
//7. get_levels for subcategory - ajax dropdown menu options
if (isset($_POST['get_level_of_subcategories']) && $_POST['get_level_of_subcategories'] != '') {
	$id = $_POST['subcategory_id'];
	if (empty($id)) {
		echo '<option value="">Level</option>';
		return false;
	}
	$sql = 'select DISTINCT(level) FROM `question` WHERE `subcategory`='.$id;
	// echo $sql;
	$db->sql($sql);
	$res     = $db->getResult();
	$options = '<option value="">select a level</option>';
	foreach ($res as $option) {
		$options .= "<option value='".$option['level']."'> Level - ".$option['level']."</option>";
	}
	echo $options;
}

//7. get_levels for subcategory - ajax dropdown menu options
if (isset($_POST['get_level_content']) && $_POST['get_level_content'] != '') {
	$id    = $_POST['subcategory_id'];
	$level = $_POST['level'];
	$sql   = "SELECT * FROM `learing_content` WHERE `subcategory` = ".$id." AND `level` =".$level;
	// echo $sql;
	$db->sql($sql);
	$res = $db->getResult();
	if (count($res) > 0) {
		$data     = $res[0];
		$response = "<textarea name='content' id='content' class='form-control'>".$data['content']."</textarea>";
	} else {
		$response = "<textarea name='content' id='content' class='form-control'></textarea>";
	}
	echo $response;
}

if (isset($_POST['levelarticle_update']) && isset($_POST['levelarticle_update'])) {
	$level       = $_POST['level'];
	...
	if (count($res) > 0) {
		...
		$db->update('learing_content', $updateData, 'content_id ='.$data['content_id']);
	} else {
		$data = array(
		...
		$db->insert('learing_content', $data);
	}
	echo 'updated';
}
